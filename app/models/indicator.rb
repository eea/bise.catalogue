class Indicator < ActiveRecord::Base
  ...
  index_name "#{Tire::Model::Search.index_prefix}indicators"
  ...
  def self.search(params)
    ...
    # Facet Filter
    indicator_filter = []
    indicator_filter << { term: { 'site.name' => params[:site] }} if params[:site].present?
    indicator_filter << { term: { 'authors.name' => params[:author] }} if params[:author].present?
    indicator_filter << { term: { 'countries.name' => params[:countries].split(/\//) }} if params[:countries].present?
    indicator_filter << { term: { 'languages.name' => params[:languages].split(/\//) }} if params[:languages].present?
    indicator_filter << { term: { 'targets.title.exact' => params[:target] }} if params[:target].present?
    indicator_filter << { term: { biographical_region: params[:biographical_region] }} if params[:biographical_region].present?
    indicator_filter << { range: { published_on: { gte: date_init , lt: date_end }}} if params[:published_on].present?
    indicator_filter << { bool: { must: { term: { approved: show_approved} }}}
    ...
    tire.search load: true, page: params[:page], per_page: 10 do
      ...
      facet 'sites' do
        terms 'site.name'
        facet_filter :and, indicator_filter unless indicator_filter.empty?
      end
